...
class WalletService {
    ...
    async payForEvent(user_id, event_id) {
        ...
        if (!event.active || (event.people_limit !== 0 && event.people_limit === event.subscribers.length - 1)) {
    ...
    async unsubscribeFromEventAndReturnMoney(user_id, event_id) {
        ...
        if (event.price !== 0) {
            user_wallet = await walletRepository.findByUserId(user._id);
            author_of_event_wallet = await walletRepository.findByUserId(author_of_event._id);
        }
