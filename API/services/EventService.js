...
class EventService {
    ...
    async getByLoginLimitSortTypePage(login, limit, sort, type, page) {
        ...
        const events = await this.#getByConditionLimitSortTypePage(condition, limit, sort, type, page);
        const total = await EventModel.countDocuments({$and: [condition, type === 'active' ? { active: true } : { active: false }]});
    ...
    async getSubscribersOfEventByEventId(id, limit, page) {
        ...
        const total = await UserModel.countDocuments({ '_id': { $in: event.subscribers } });

        return {subscribers: subscribersDto, total};
    }
    ...
    async updateById(id, name, description, country, city, street, house_number,
        ...
        if (active !== undefined && event.active !== active) {
            if(Date.now() > event.time_end) {
                throw ErrorHandler.BadRequest("You can't change status of finished event");
            }
            event.active = active;
        }
